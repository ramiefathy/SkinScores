...
async function analyzeUI() {
  ...
  try {
    ...
    const toolCards = await page
      .locator('[class*="ToolCard"], [class*="tool-card"], .MuiCard-root')
      .count();
    const searchBar = await page
      .locator('input[placeholder*="Search"], input[placeholder*="search"]')
      .count();
    const categories = await page
      .locator('[role="button"][aria-label*="Category"], .MuiChip-root')
      .count();
    findings.push(
      `Library Page: ${toolCards} tool cards, Search bar: ${searchBar > 0}, Categories: ${categories}`,
    );
    ...
    if (toolHref) {
      ...
      findings.push(
        `Calculator Page: ${inputFields} inputs, Calculate button: ${calculateButton > 0}, References: ${references}`,
      );
    }
