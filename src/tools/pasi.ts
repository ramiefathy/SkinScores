...
export const pasiTool: Tool = {
  ...
  icon: Gauge,
  rationale:
    'The PASI combines both the extent of body surface area involvement and the severity of key clinical features (erythema, induration, and desquamation) into a single composite score.\n\n**Formula: PASI = 0.1(E+I+D)h × Ah + 0.2(E+I+D)u × Au + 0.3(E+I+D)t × At + 0.4(E+I+D)l × Al**\n\n**Regional Weights:** Head (h): 10% (0.1), Upper limbs (u): 20% (0.2), Trunk (t): 30% (0.3), Lower limbs (l): 40% (0.4)\n\n**Severity Parameters (0-4):** Erythema (E) = redness, Induration (I) = thickness/hardening, Desquamation (D) = scaling\n\n**Area Score (0-6):** 0 = 0%, 1 = 1-9%, 2 = 10-29%, 3 = 30-49%, 4 = 50-69%, 5 = 70-89%, 6 = 90-100%\n\nMaximum score: 72. The PASI score represents absolute disease severity at a single time point.',
  clinicalPerformance:
  ...
  formSections: (['h', 'u', 't', 'l'] as const).map((regionAbbr) => {
    ...
    return {
      ...
      gridCols: 4,
      description: `Assess Erythema (redness), Induration (thickness/hardening), Scaling (desquamation), and Area for the ${regionFullName} region. This region accounts for ${bsaPercent[regionAbbr]}% of Body Surface Area (weight = ${regionMultiplier}).`,
      inputs: [
        {
          id: `E_${regionAbbr}`,
          label: `Erythema (E)`,
  ...
  calculationLogic: (inputs) => {
    ...
    interpretation +=
      '\n\n**Severity Bands:** <10 Mild; 10-20 Moderate; >20 Severe\n**Responder Definitions:** PASI 50 = ≥50% reduction from baseline; PASI 75 = ≥75% reduction (FDA benchmark for efficacy); PASI 90 = ≥90% reduction (emerging standard with newer biologics); PASI 100 = Complete clearance\n**Clinical Note:** Consider using BSA and PGA alongside PASI for comprehensive assessment. PASI 75 is the current FDA benchmark for treatment efficacy.';
