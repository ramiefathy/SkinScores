...
export const sliccCriteriaTool: Tool = {
  ...
  description: "Classification criteria for Systemic Lupus Erythematosus (SLE), primarily for research. Diagnosis requires at least 4 criteria (with at least one clinical and one immunologic) OR biopsy-proven lupus nephritis with positive ANA or anti-dsDNA.",
  condition: "Lupus",
  ...
  calculationLogic: (inputs) => {
    ...
    const clinicalCount = sliccClinicalCriteria.reduce((sum, crit) => sum + (Number(inputs[crit.id]) || 0), 0);
    const immunologicCount = sliccImmunologicCriteria.reduce((sum, crit) => sum + (Number(inputs[crit.id]) || 0), 0);
