...
const behcetFormSections: FormSectionConfig[] = [
  {
    id: 'behcet_required',
    label: 'Recurrent oral ulceration (≥3 times in 12 months)',
    type: 'select',
  ...
  },
  {
    ...
    title: "Plus two of the following:",
    gridCols: 1,
    inputs: [
        { id: "behcet_genital", label: "Recurrent genital ulceration", type: 'select', options: behcetOptions, defaultValue: 0, validation: getValidationSchema('select', behcetOptions) },
        { id: "behcet_eye", label: "Eye lesions (e.g., uveitis, retinal vasculitis)", type: 'select', options: behcetOptions, defaultValue: 0, validation: getValidationSchema('select', behcetOptions) },
        { id: "behcet_skin", label: "Skin lesions (e.g., erythema nodosum, pseudofolliculitis)", type: 'select', options: behcetOptions, defaultValue: 0, validation: getValidationSchema('select', behcetOptions) },
        { id: "behcet_pathergy", label: "Positive pathergy test", type: 'select', options: behcetOptions, defaultValue: 0, validation: getValidationSchema('select', behcetOptions) }
    ]
  }
];
...
export const behcetCriteriaTool: Tool = {
  ...
  description: "Diagnostic/classification criteria for Behçet’s disease. Requires recurrent oral ulceration plus any two of the following: recurrent genital ulceration, eye lesions, skin lesions, or a positive pathergy test.",
  ...
  calculationLogic: (inputs) => {
    ...
    return {
      ...
      details: {
        ...
        "Recurrent genital ulceration": (Number(inputs.behcet_genital) || 0) === 1 ? "Present" : "Absent",
        "Eye lesions": (Number(inputs.behcet_eye) || 0) === 1 ? "Present" : "Absent",
        "Skin lesions": (Number(inputs.behcet_skin) || 0) === 1 ? "Present" : "Absent",
        "Positive pathergy test": (Number(inputs.behcet_pathergy) || 0) === 1 ? "Present" : "Absent",
