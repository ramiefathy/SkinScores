...
const nappaClinFormSections: FormSectionConfig[] = Array.from({ length: 4 }, (_, i) => ({
    ...
    gridCols: 2,
    inputs: [
        { id: `nail_${i+1}_matrix`, label: "Matrix Score (0-4)", type: 'number', min: 0, max: 4, defaultValue: 0, description:"Quadrants w/ pitting, leukonychia, red spots, or crumbling.", validation: getValidationSchema('number', [], 0, 4) },
        { id: `nail_${i+1}_bed`, label: "Bed Score (0-4)", type: 'number', min: 0, max: 4, defaultValue: 0, description: "Quadrants w/ onycholysis, splinter hemorrhages, oil drop, or hyperkeratosis.", validation: getValidationSchema('number', [], 0, 4) }
    ]
}));
...
export const nappaClinTool: Tool = {
  ...
  acronym: "NAPPA-CLIN",
  description: "A clinician-rated severity index for nail psoriasis, which is a shortened version of NAPSI, assessing only four digits instead of all twenty. It is one of three components of the full NAPPA tool, which also includes patient-reported QoL (NAPPA-QoL) and benefit (NAPPA-PBI) indices.",
