...
const buildFormSchema = (tool?: Tool) => {
  ...
  flattenInputs(tool.formSections).forEach((input) => {
    if (input.validation) {
      ...
      shape[input.id] = z.union([z.string(), z.number(), z.boolean()]).optional();
    }
  });
...
const formatForEMR = (result: CalculationResult, toolName: string, patientId?: string): string => {
  const timestamp = new Date();
  const formattedDate = timestamp.toLocaleDateString('en-US');
  const formattedTime = timestamp.toLocaleTimeString('en-US');
  const score = typeof result.score === 'number' ? result.score.toFixed(2) : result.score;

  // Build markdown table for assessment info
  let emrText = `## Clinical Assessment Result - ${toolName}\n\n`;

  // Create info table
  emrText += `| Field | Value |\n`;
  emrText += `|-------|-------|\n`;
  emrText += `| **Tool** | ${toolName} |\n`;
  emrText += `| **Date** | ${formattedDate} |\n`;
  emrText += `| **Time** | ${formattedTime} |\n`;
  emrText += `| **Score** | **${score}** |\n`;

  if (patientId) {
    emrText += `| **Patient ID** | ${patientId} |\n`;
  }

  emrText += `\n### Clinical Interpretation\n\n`;
  emrText += result.interpretation;

  // If there are details, format them as a table
  if (result.details && Object.keys(result.details).length > 0) {
    emrText += '\n\n### Assessment Components\n\n';
    emrText += '| Component | Value |\n';
    emrText += '|-----------|-------|\n';

    Object.entries(result.details).forEach(([key, value]) => {
      const formattedKey = key.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
      const formattedValue = formatDetailValue(value);
      emrText += `| ${formattedKey} | ${formattedValue} |\n`;
    });
  }

  emrText += '\n\n---\n*Generated by SkinScores Clinical Assessment Tool*';
        ...
        <Grid item xs={12} lg={8}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <Paper elevation={0} sx={{ p: 4, mb: 3 }}>
              ...
              {(() => {
                // Check if tool has many sections (like AAS28 with 28 days)
                const sectionCount = tool.formSections.length;
                const isLongForm = sectionCount > 30; // Tools with many fields like AAS28

                if (isLongForm && tool.id === 'aas28') {
                  // Special handling for AAS28 - group by week
                  const weeks: { label: string; sections: FormSectionConfig[] }[] = [
                    { label: 'Instructions', sections: [tool.formSections[0]] },
                    { label: 'Week 1 (Days 1-7)', sections: tool.formSections.slice(1, 36) },
                    { label: 'Week 2 (Days 8-14)', sections: tool.formSections.slice(36, 71) },
                    { label: 'Week 3 (Days 15-21)', sections: tool.formSections.slice(71, 106) },
                    { label: 'Week 4 (Days 22-28)', sections: tool.formSections.slice(106) },
                  ];
                  ...
                  return (
                    <>
                      ...
                      >
                        {weeks.map((week, idx) => (
                          <Tab key={idx} label={week.label} />
                        ))}
                      </Tabs>
                      <Box sx={{ pt: 2 }}>
                        <Stack spacing={3}>
                          {weeks[activeTab].sections.map((section, index) => (
                            <FormSection
                ...
                } else if (sectionCount > 15) {
                  // For other tools with many sections, use accordions
                  return (
                    <Stack spacing={2}>
                      {tool.formSections.map((section, index) => {
                        const sectionId = isInputGroup(section)
                          ? section.id
                          : `${section.id}-${index}`;
                        const title = isInputGroup(section)
                          ? section.title
                        ...
                        return (
                          ...
                          >
                            ...
                            <AccordionDetails>
                              <FormSection section={section} control={control} errors={errors} />
                            </AccordionDetails>
                          </Accordion>
                        );
                      })}
                  ...
                  // Regular tools - display normally
                  return (
                    <Stack spacing={4}>
                      {tool.formSections.map((section, index) => (
                        <FormSection
          ...
          <Snackbar
            ...
            message="Result copied to clipboard for EMR"
